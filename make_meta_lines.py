...
def get_author_selections_for_plot(author_set):
    temp_list = [i for i in range(1, 81)]
...
def make_simple_confusion_lines(df, author_set, author_list):
    ...
    # Extract false positives >= 0.75 for further processing.
    filtered_df = df[(df['threshold'] >= 0.75) & (df['same_author'] == 'False Positive')]
    with open(f'./projects/{project_name}/results/fp-high-threshold.csv', 'w') as the_csv:
        filtered_df.to_csv(the_csv, index=False)
